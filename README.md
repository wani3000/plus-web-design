...
- Tablet (`768px ~ 1100px`) now uses a single `앱 다운로드` button in the fixed header and CTA section, and reuses the mobile footer layout instead of the desktop footer layout.
- Tablet (`768px ~ 1100px`) horizontal gutters are standardized to `20px` by using section-level side padding with `inner` containers set to `width: 100%`.
